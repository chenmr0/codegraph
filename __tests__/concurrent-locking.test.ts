  ...
  it('concurrent read tool calls (mixed projectPath) all succeed without "database is locked"', async () => {
    ...
    try {
      const calls: Promise<{ content: Array<{ text: string }>; isError?: boolean }>[] = [
        handler.execute('search', { query: 'helper' }),
        handler.execute('search', { query: 'helper', projectPath: root }),
        handler.execute('callers', { symbol: 'helper', projectPath: root }),
        handler.execute('callees', { symbol: 'main' }),
        handler.execute('files', { projectPath: root }),
        handler.execute('status', { projectPath: root }),
      ];
