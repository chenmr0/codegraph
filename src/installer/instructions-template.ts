...
- Find one symbol by name/signature → \`codegraph_search\`; find 2–8 symbols with ONE \`codegraph_search(queries=[...])\` call so true misses share one raw scan. Add \`includeCode="if_unique"\` when a unique logical result should include declaration and definition source immediately; a wrong qualified owner is corrected from structured leaf candidates before raw scanning.
- Read one known symbol → \`codegraph_node(symbol=..., includeCode=true)\`; read 2–8 known targets → ONE native \`codegraph_node(targets=[...])\` batch, which returns the same merged implementation bundle as context.
...
- Unexpected empty graph results include grep-equivalent current-source evidence when safe: trust \`CONFIRMED_ABSENT\`; \`DECLARATION_ONLY\` means the exact overload has a declaration but no paired indexed definition and the complete identifier occurrence evidence is already shown; treat \`RAW_MATCHES\` as an index/parser-gap signal; narrow scope after \`INCONCLUSIVE\`. Repeated identical evidence may reuse an unchanged watched-source epoch cache.
