...
- **Never pass these to \`codegraph_search\`**: hex values (\`0x4237F001\` — search for the constant name instead), space-separated commands/debug flags (\`ADD TRMDBG\` — search \`TRMDBG\` alone), dot-separated proto/package references (\`RepAlloc.Rsp\` — search \`RepAlloc\` or \`Rsp\` or the joined form \`RepAllocRsp\`), or English questions (\`how does auth work?\`).
