...
- Known symbol/location/signature → \`codegraph_search\` (pass a symbol name or callable signature, not a natural-language question). Search several names with ONE \`queries=[{query,...}, ...]\` batch; all true misses share one multi-pattern raw-source scan. If implementation is needed, set \`includeCode: "if_unique"\`; one logical C/C++ overload returns both declaration and definition source. If only a qualified owner is wrong but the exact leaf exists, search returns structured owner-correction candidates and skips a repository scan.
